...
def gen_doc_pre(cur, title, mutation_prob, content:list[Heading]):
    ...
    if not cur.childlist:
        if dep == [-1]:
            ...
            content_jsonStr:str = contentExpert.content_to_jsonStr(content)
            # 获取 last_heading 的文本内容
            last_heading:str = get_last_heading(id, content)
            # 先生成 text
            new_text = write_without_dep(title, heading, content_jsonStr, digest, last_heading, retrieved_knowledge)  # 先生成 text
            # 再更新 digest
            ...
            # 先生成text
            new_text = write_with_dep(title, heading, content_jsonStr, digest, last_heading, dep_text, retrieved_knowledge)
            # 再更新 digest
            digest = write_digest(title, heading, new_text, digest, content)   
            # 最后存储写作好的文本和对应的依赖文本
            cur.heading.text = new_text
...
def gen_doc_mutation(cur, title, mutation_prob, content):
    ...
    if cur.heading.level > 0 and mutation(mutation_prob):
        ...
        # 先生成text
        new_text = write_mutation(title, heading, content_jsonStr, digest, last_heading, dep_text, retrieved_knowledge)
        # 再更新 digest
        digest = write_digest(title, heading, new_text, digest, content)  
        # 最后存储写作好的文本和对应的依赖文本
        cur.heading.dep_text = dep_text
