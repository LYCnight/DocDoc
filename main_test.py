...
from core.prompt import WRITE_PROMPT
from core.prompt import WRITER_INSTRUCTOR_PROMPT_ROLE, WRITER_INSTRUCTOR_PROMPT_CONSTRAINT, WRITER_INSTRUCTOR_PROMPT_TITLE_AND_CONTENT, WRITER_INSTRUCTOR_PROMPT_EXAMPLE, WRITER_INSTRUCTOR_PROMPT_END
from core.prompt import FORMAT_CONTROLLER_PROMPT
...
while conQue.empty() != True:
    ...
    for i, para_guide_dict in enumerate(para_guide_list, start=1):
        ...
        write_prompt = PromptTemplate(
            input_variables=["para_guide"],
            template = WRITE_PROMPT
        ).format(para_guide=para_guide) 
        # 打印 写作专家 的提示词
        if debug == True:
        ...
        # 获得 写作专家 的响应
        para_no_fromat = llm(write_prompt)
        # 打印 写作专家 的响应
        if debug == True:
            print("*" * 50 + f"`{current_headline}`-第{i}段的内容-str格式" + "*" * 50)
            print(para_no_fromat)

        # # 构造 格式控制员 的提示词
        # format_controller_prompt = PromptTemplate(
        #     input_variables=["text_to_be_formatted"],
        #     template = FORMAT_CONTROLLER_PROMPT
        # ).format(text_to_be_formatted=para_no_fromat)     
        
        # # 获得 格式控制员 的响应
        # para_fromatted = llm(format_controller_prompt)
        # # 打印 格式化后的 段落
        # if debug == True:
        #     print("*" * 50 + f"`{current_headline}`-第{i}段的内容-str格式-格式化后" + "*" * 50)
        #     print(para_fromatted)

        # # 拼装上下文 - 有格式控制员
        # context += para_fromatted
        
        # 拼装上下文 - 无格式控制员
        context += para_no_fromat

    # print("*" * 50 + f"`{current_headline}`的内容-str格式-格式化后" + "*" * 50)   # 有格式控制员
    print("*" * 50 + f"`{current_headline}`的内容-str格式" + "*" * 50)   # 无格式控制员
    print(context)

    # 拼装全文
    full_text += context
    
print("*" * 50 + f"`完整全文-str格式" + "*" * 50) 
print(full_text)
